"""History-based u (upper bound) for y[i,j].

This module loads past usage counts (or any precomputed u) from a CSV and
...
- The solver still caps u by demand.
"""
...
def load_history_u(csv_path: str | Path) -> HistoryU:
    path = Path(csv_path)
    mapping: Dict[Key, int] = {}

    if not path.exists():
        return HistoryU(mapping={})
    ...
    with path.open("r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            u_raw = row.get("u") or row.get("ub") or row.get("UB") or row.get("qty") or row.get("QTY")

            if not piece or not leather or u_raw is None:
                continue

            try:
                u = int(float(str(u_raw).strip()))
            except Exception:
                continue
            ...
            mapping[(piece, leather)] = u

    return HistoryU(mapping=mapping)
